	...
	if (pack) {
		...
		switch (pack->header->msg) {
			...
			case MRIM_CS_NEW_MAIL:
				...
				break;
#ifdef ENABLE_FILES
			// Should be CLEANED OUT! (TODO) All the way ahead!
			case MRIM_CS_FILE_TRANSFER:

				mrim_xfer_got_rq(pack, mrim);
				break;
				
			case MRIM_CS_FILE_TRANSFER_ACK:
				
				mrim_xfer_ack(pack, mrim);
				break;

			case MRIM_CS_PROXY_ACK:
			
				mrim_xfer_proxy_ack(pack, mrim);
				break;
...
void mrim_open_myworld_url(MrimData *mrim, gchar *user_name, gchar *fmt) {
	g_return_if_fail(is_myworld_able(user_name) == TRUE);
